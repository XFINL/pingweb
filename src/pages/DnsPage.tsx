...
import type { DnsNodeResult } from "@/components/DnsResult";
...
export default function DnsPage() {
  ...
  const [results, setResults] = useState<DnsNodeResult[] | null>(null);
  ...
  const doDetect = async () => {
    ...
    try {
      const res = await fetch("/api/dns", {
        ...
        body: JSON.stringify({ target: target.trim(), v6: false }),
      });
    ...
    <div className="min-h-screen bg-black flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center px-6 pt-24 pb-12">
        <div className="w-full max-w-6xl">
          ...
          {/* Results */}
          {!loading && results && (
            <DnsResult results={results} target={target} completedAt={completedAt!} />
          )}
        </div>
      </main>
    </div>
